...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF and self.future_price != 0:
            ...
            # Go 1 dollar above and below current best bid and ask
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            final_volume = min(hedge_volume, POSITION_LIMIT + self.hedge_position)
            if final_volume > 0:
                order_id = next(self.order_ids)
                self.send_hedge_order(order_id, Side.ASK, MIN_BID_NEAREST_TICK, final_volume)
            ...
            if final_volume > 0:
                order_id = next(self.order_ids)
                self.send_hedge_order(order_id, Side.BID, MAX_ASK_NEAREST_TICK, final_volume)
