...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if self.orders_history["volume"] > 0:
                ...
                new_vol = min(ask_volumes[0], self.orders_history["volume"])

                if ask_prices[0] > self.orders_history["price"] and self.position <= POSITION_LIMIT - new_vol:
                    self.ask_id = next(self.order_ids)
                    print(min(self.orders_history["volume"], bid_volumes[0]))
                    print(bid_prices[0])
                    self.send_insert_order(self.ask_id, Side.ASK, ask_prices[0], new_vol,
                                           Lifespan.
                                           FILL_AND_KILL)
                    self.asks.add(self.ask_id)
                    print(min(self.orders_history["volume"], ask_volumes[0]))
                    print(ask_prices[0])

            elif self.orders_history["volume"] < 0:
                new_vol = min(bid_volumes[0], abs(self.orders_history["volume"]))
                if bid_prices[0] < self.orders_history["price"] and self.position >= POSITION_LIMIT - new_vol:
                    self.bid_id = next(self.order_ids)
                    print(min(self.orders_history["volume"], bid_volumes[0]))
                    print(bid_prices[0])
                    self.send_insert_order(self.bid_id, Side.BUY, bid_prices[0], new_vol,
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if (volume > 0 and self.orders_history["volume"] > 0) or (volume < 0 and self.orders_history["volume"] < 0):
            # Just find average
            self.orders_history["price"] = (abs(volume) * price + abs(self.orders_history["volume"] * self.orders_history["price"])) // new_vol
